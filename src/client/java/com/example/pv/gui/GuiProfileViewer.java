package com.example.pv.gui;

import com.example.pv.api.ProfileFetcher;
import com.example.pv.util.*;
...
public class GuiProfileViewer extends Screen {
	...
	public GuiProfileViewer(String username) {
		...
		com.example.pv.util.RepoManager.init();
	}
	...
	private void drawSearchBox(GuiGraphics g) {
		...
		if (searchInput.isEmpty()) {
			...
			String comp = com.example.pv.util.PlayerNameCache.complete(searchInput);
			if (comp != null) {
	...
	private void renderDungeonPage(GuiGraphics g) {
		...
		com.example.pv.util.SkillXp.Level cl =
			com.example.pv.util.SkillXp.getLevel(cataXp, com.example.pv.util.SkillXp.Type.CATACOMBS, com.example.pv.util.SkillXp.CATACOMBS_OVERFLOW_MAX);
		...
		if (cataCalcResult != null) {
			...
			PvRender.drawString(g, "Until " + (cl.level+1) + ": " + formatNumber(com.example.pv.util.SkillXp.xpForLevel(cl.level+1, com.example.pv.util.SkillXp.Type.CATACOMBS) - cataXp), lx + 4, ly + 41, 0xAAAAAA, true);
		}
		...
		for (int i = 0; i < bosses.length; i++) {
			...
			net.minecraft.resources.ResourceLocation head = com.example.pv.util.BossHeads.getFloor(i + 1);
			if (head != null) {
				try {
		...
		for (int i = 0; i < classNames.length; i++) {
			...
			com.example.pv.util.SkillXp.Level lvl =
				com.example.pv.util.SkillXp.getLevel(cxp, com.example.pv.util.SkillXp.Type.CATACOMBS, 50);
			avgLevel += lvl.level;
			int cy = ry + 24 + i * 22;
			...
			if (lvl.level >= 50) {
				...
				double next = com.example.pv.util.SkillXp.xpForLevel(lvl.level + 1, com.example.pv.util.SkillXp.Type.CATACOMBS);
	...
	private void doCataCalculate() {
		...
		try {
			...
			if (target < 1 || target > com.example.pv.util.SkillXp.CATACOMBS_OVERFLOW_MAX) { cataCalcResult = "Enter 1-200"; return; }
			JsonObject selected = getSelectedProfile();
			JsonObject d = selected.getAsJsonObject("dungeons");
			double cataXp = getDouble(d, "catacombs_xp", 0);
			double needed = com.example.pv.util.SkillXp.xpForLevel(target, com.example.pv.util.SkillXp.Type.CATACOMBS);
	...
	private void renderCollectionsPage(GuiGraphics g) {
		...
		for (int k = colPage * perPage, j = 0; k < Math.min((colPage + 1) * perPage, list.size()); k++, j++) {
			...
			com.example.pv.util.CollectionIcon.draw(g, id, x, y);
	...
	private void renderMinions(GuiGraphics g, String cat, String label) {
		...
		for (int i = 0; i < mins.length; i++) {
			...
			String skull = com.example.pv.util.RepoItems.skull(internal);
			if (skull != null) com.example.pv.util.ItemIcon.drawSkull(g, skull, x, y);
			...
			if (hoverMouseX >= x && hoverMouseX <= x + slot && hoverMouseY >= y && hoverMouseY <= y + slot) {
				List<String> lore = com.example.pv.util.RepoItems.lore(internal);
				String dn = com.example.pv.util.RepoItems.displayName(internal);
	...
	private void renderPetsPage(GuiGraphics g) {
		...
		if (petSelected >= 0 && petSelected < list.size()) {
			...
			String dn = com.example.pv.util.RepoPets.displayName(type, tier, lvl.level);
			...
			String skullTex = com.example.pv.util.RepoPets.skull(type, tier);
			ItemStack head = skullTex != null ? com.example.pv.util.ItemIcon.skullStack(skullTex)
			...
			String msg = com.example.pv.util.RepoManager.isReady() ? "§7Select a pet" : "§7Downloading repo...";
		...
		for (int k = petPage * perPage, j = 0; k < Math.min((petPage + 1) * perPage, list.size()); k++, j++) {
			...
			g.fill(x, y, x + slot, y + slot, 0xFF202020);
			String gSkull = com.example.pv.util.RepoPets.skull(type, tier);
			if (gSkull != null) com.example.pv.util.ItemIcon.drawSkull(g, gSkull, x, y);
			...
			if (hoverMouseX >= x && hoverMouseX <= x + slot && hoverMouseY >= y && hoverMouseY <= y + slot) {
				List<String> repoLore = com.example.pv.util.RepoPets.lore(type, tier, lvl.level);
				StringBuilder sb;
				if (repoLore != null && !repoLore.isEmpty()) {

					String held = p.has("heldItem") && !p.get("heldItem").isJsonNull() ? p.get("heldItem").getAsString() : null;
					List<String> heldLines = com.example.pv.util.RepoPets.heldItemLines(held);
					...
					String cat = com.example.pv.util.PetStats.category(type);
					if (cat != null) sb.append("\n§7").append(cap(cat)).append(" Pet");
					for (String line : com.example.pv.util.PetStats.statLines(type, tier, lvl.level)) sb.append("\n").append(line);
					String held = p.has("heldItem") && !p.get("heldItem").isJsonNull() ? p.get("heldItem").getAsString() : null;
	...
	private void renderMiningPage(GuiGraphics g) {
		...
		int hotmLevel = com.example.pv.util.Hotm.level(exp);
		int barY = guiT + 22;
		g.fill(lx - 4, barY - 4, lx + 134, barY + 22, 0xC0000000);
		PvRender.drawString(g, "§d❤ HOTM §f" + hotmLevel + "§8/§7" + com.example.pv.util.Hotm.maxLevel(), lx, barY, 0xFF55FF, true);
		int pbX = lx, pbY = barY + 11, pbW = 126;

		double overall = Math.min(1.0, (hotmLevel - 1 + com.example.pv.util.Hotm.progress(exp)) / com.example.pv.util.Hotm.maxLevel());
		...
		for (com.example.pv.util.Hotm.Perk p : com.example.pv.util.Hotm.perks().values()) {
	...
	private ItemStack hotmIcon(com.example.pv.util.Hotm.Perk p, int lvl) {
	...
	private int killsFor(JsonObject kills, com.example.pv.util.BestiaryData.Mob m) {
		int s = 0;
	...
	private void renderBestiaryPage(GuiGraphics g) {
		...
		int guiL = guiLeft, guiT = guiTop;
		java.util.List<com.example.pv.util.BestiaryData.Island> islands = com.example.pv.util.BestiaryData.islands();
		if (islands == null || islands.isEmpty()) {
			String msg = com.example.pv.util.RepoManager.isReady() ? "§cNo bestiary data" : "§7Downloading repo...";
		...
		for (int i = 0; i < islands.size(); i++) {
			com.example.pv.util.BestiaryData.Island is = islands.get(i);
			int x = ix + i * 20;
			...
			if (is.iconTexture != null) com.example.pv.util.ItemIcon.drawSkull(g, is.iconTexture, x, iy);
		...
		int totalTiers = 0;
		for (com.example.pv.util.BestiaryData.Island is : islands)
			for (com.example.pv.util.BestiaryData.Mob m : is.mobs)
				totalTiers += com.example.pv.util.BestiaryData.tier(killsFor(kills, m), m.bracket);

		com.example.pv.util.BestiaryData.Island isl = islands.get(bestiaryIsland);
		...
		for (int i = 0; i < isl.mobs.size(); i++) {
			com.example.pv.util.BestiaryData.Mob m = isl.mobs.get(i);
			int k = killsFor(kills, m);
			int tier = com.example.pv.util.BestiaryData.tier(k, m.bracket);
			int maxT = com.example.pv.util.BestiaryData.maxTier(m.cap, m.bracket);
			if (tier > 0) found++;
			...
			if (m.texture != null) com.example.pv.util.ItemIcon.drawSkull(g, m.texture, x, y);
			...
			if (hoverMouseX >= x && hoverMouseX <= x + 16 && hoverMouseY >= y && hoverMouseY <= y + 16) {
				...
				int next = com.example.pv.util.BestiaryData.nextThreshold(k, m.bracket, m.cap);
	...
	private void renderTrophyPage(GuiGraphics g) {
		...
		if (!com.example.pv.util.RepoManager.isReady()) {
		...
		for (int i = 0; i < 4; i++) {
			int y = guiT + 42 + i * 12;
			String hs = com.example.pv.util.RepoItems.skull(TrophyFishData.RANK_HELMETS[i]);
			if (hs != null) com.example.pv.util.ItemIcon.drawSkull(g, hs, guiL + 11, y);
	...
	private void renderRiftPage(GuiGraphics g) {
		...
		if (!hasItems(items)) {
			...
			for (int i = 0; i < pageItems.size(); i++) {
				...
				com.example.pv.util.ItemIcon.draw(g, it, ix, iy);
	...
	private void drawLoadoutSlot(GuiGraphics g, ItemData it, int x, int y) {
		...
		com.example.pv.util.ItemIcon.draw(g, it, x, y);
		if (hoverMouseX >= x && hoverMouseX <= x + 16 && hoverMouseY >= y && hoverMouseY <= y + 16) {
	...
	private void drawLeftInfo(
		...
		com.example.pv.util.PlayerModelRender.render(
	...
	private void drawCenterPanel(GuiGraphics g, int x, int y, double sbXp, double socialXp) {
		...
		com.example.pv.util.SkillXp.Level social =
			com.example.pv.util.SkillXp.getLevel(socialXp, com.example.pv.util.SkillXp.Type.SOCIAL, 25);
	...
	private void drawSkill(GuiGraphics g, String name, String skillKey, double xp, int maxLevel, int x, int y, int color) {
		com.example.pv.util.SkillXp.Level level = com.example.pv.util.SkillXp.getLevel(xp, com.example.pv.util.SkillXp.typeFor(skillKey), maxLevel);
