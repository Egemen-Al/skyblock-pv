...
package com.eggman.pv.weight.lily;

import com.google.gson.JsonElement;
import com.google.gson.JsonObject;
import com.eggman.pv.data.Level;
import com.eggman.pv.weight.weight.DungeonsWeight;
...
public class LilyDungeonsWeight extends DungeonsWeight {
	...
	public LilyDungeonsWeight(Map<String, Level> player, JsonObject profileJson) {
		super(player);
	...
	public void getDungeonWeight() {
		...
		if (catacombs.level != 0) {
			if (catacombs.totalXp < CATACOMBS_LEVEL_50_XP) {
				weightStruct.add(
					new WeightStruct(
						PvCalc.getElement(Constants.WEIGHT, "lily.dungeons.overall").getAsDouble() *
	...
	public void getDungeonCompletionWeight(String cataMode) {
		JsonObject dungeonsCompletionWorth = PvCalc.getElement(Constants.WEIGHT, "lily.dungeons.completion_worth").getAsJsonObject();
		JsonObject dungeonsCompletionBuffs = PvCalc.getElement(Constants.WEIGHT, "lily.dungeons.completion_buffs").getAsJsonObject();
		...
		if (cataMode.equals("normal")) {
			if (PvCalc.getElement(profileJson, "dungeons.dungeon_types.catacombs.tier_completions") == null) {
				return;
			}

			for (Map.Entry<String, JsonElement> normalFloor : PvCalc
			...
			if (PvCalc.getElement(profileJson, "dungeons.dungeon_types.master_catacombs.tier_completions") == null) {
				return;
			}

			for (Map.Entry<String, JsonElement> masterFloor : PvCalc
